...
- Billing activation remains paused at operator request, not a blocker for unrelated development. Do not prompt for Stripe setup until hosted payment work is explicitly resumed; hosted verification is still required before charging. See `artifacts/api-server/docs/billing-sandbox-verification.md`.
- Tiered billing separates tier, cadence, and currency; immutable verified coverage determines capabilities and monthly ceilings without resetting usage. Historical Prices remain recoverable. See `billing-activation-review.md`, `billing-notification-operations.md`, and `billing-test-clock.md` in the same docs directory for operator approvals and provider-free versus hosted evidence boundaries.
